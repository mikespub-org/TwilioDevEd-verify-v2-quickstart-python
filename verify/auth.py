...
def start_verification(to, channel='sms'):
    ...
    verification = client.verify.services(service).verifications.create(
...
def check_verification(phone, code):
    ...
    try:
        ...
        if verification_check.status == "approved":
            db = get_db()
            db.execute("UPDATE user SET verified = 1 WHERE phone_number = ?", (phone,))
            db.commit()
        ...
        flash(f"Error validating code: {e}")
